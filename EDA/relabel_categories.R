...
relabel_likert <- function(variable) {
  ...
  # Use case_when for robust relabeling and convert to an ordered factor
  factor(
    case_when(
...
# The pipe (%>%) sends the model_data dataframe to the mutate function,
# and the result is assigned back to model_data.
...
# --- View the summary of the new relabeled data ---
# This will show the new factor columns with their labeled counts.
cat("\n--- Data Summary after Relabeling ---\n")
summary(model_data)