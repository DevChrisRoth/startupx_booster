# ==============================================================================
# UTILS: MODELING HELPERS
# Purpose: Helper functions for training, evaluation, and reporting
# ==============================================================================
...
calculate_class_weights <- function(data, target_col) {
  # Ensure target is binary
  counts <- table(data[[target_col]])
  if(length(counts) != 2) stop("Target must be binary for this weight function")
  
  count_no <- sum(data[[target_col]] == "No")
  count_yes <- sum(data[[target_col]] == "Yes")
  
  # XGBoost: Scale_pos_weight (Ratio of Negative to Positive)
  xgb_weight_value <- count_no / count_yes
  
  # Random Forest: Class Weights (Inverse frequency, normalized)
  rf_class_weights <- c(
    "No" = (1 / count_no) * (count_no + count_yes) / 2,
    "Yes" = (1 / count_yes) * (count_no + count_yes) / 2
  )
  
  return(list(xgb = xgb_weight_value, rf = rf_class_weights))
}
